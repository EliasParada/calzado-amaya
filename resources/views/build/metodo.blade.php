...
@section('title')
<title>Método de pago | Calzado Amaya</title>
<style>
...
<div class="container mx-auto p-8">
    ...
    <div class="container mx-auto flex flex-col md:flex-row justify-around">
        <form action="{{ route('cobrar') }}" method="POST" class="w-full md:w-1/2 pl-4 flex items-start justify-start flex-col">
            @csrf
            <input type="hidden" name="envio" required class="hidden" value="{{ $envio }}">
            <input type="hidden" name="nombre" required class="hidden" value="{{$contacto['nombre']}}">
            <input type="hidden" name="apellido" required class="hidden" value="{{$contacto['apellido']}}">
            <input type="hidden" name="telefono" required class="hidden" value="{{$contacto['telefono']}}">
            <input type="hidden" name="correo" required class="hidden" value="{{$contacto['correo']}}">
            <input type="hidden" name="direccion" required class="hidden" value="{{$contacto['direccion']}}">
            <textarea name="detalle" class="hidden">{{$contacto['detalle']}}</textarea>

            <h2 class="text-xl font-bold mb-4">Resumen del pedido</h2>
            <div class="w-full flex flex-col gap-2">
                <input type="radio" name="metodo" id="pagadito" value="PAGADITO" class="hidden" required>
                <label for="pagadito" class="block mb-4 border-2 border-black flex gap-4 items-center">
                    <div class="w-14 h-14 m-2 bg-white">
                        <img src="{{ asset('img/pagadito.png') }}" alt="Pagadito" srcset="">
                    </div>
                    <div>
                        <span class="block mb-1">Pagar en linea</span>
                        <span class="block mb-1 font-bold">Paga mediante la plataforma de Pagadito.</span>
                    </div>
                </label>

                <input type="radio" name="metodo" id="efectivo" value="EFECTIVO" class="hidden" required>
                <label for="efectivo" class="block mb-4 border-2 border-black flex gap-4 items-center">
                    <div class="w-14 h-14 m-2 bg-white">
                        <img src="{{ asset('img/efectivo.png') }}" alt="Pago en efectivo" srcset="">
                    </div>
                    <div>
                        <span for="telefono" class="block mb-1">Pagar en efectivo</span>
                        <span for="telefono" class="block mb-1 font-bold">Paga cuando llegue tu pedido.</span>
                    </div>
                </label>
                <button type="submit" class="bg-white text-black px-4 py-2 border-2 border-black hover:bg-black hover:text-white mt-4 w-full">Finalizar</button>
            </div>
        </form>
